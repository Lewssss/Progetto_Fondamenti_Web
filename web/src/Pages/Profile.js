...
function Profile({ sidebar, onMessageUser }) {
  ...
  const posts = allPosts
    .filter((post) => String(post.authorId) === String(targetId))
    .slice(0, 6); //dato che da backend ordiniamo per ordine recente, i primi 6 sono i piu nuovi
    ...
    <div className="profile">
      <div className="user-header">
        <div className="profile-user">
          <div
            className={`profile-userimg-wrapper ${userStories.length > 0 ? "has-stories" : ""}`}
            onClick={() => {
        ...
        <div className="profile-actions">
          {isOwnProfile ? (
            ...
            <>
              <button type="button" id="Follow" onClick={handleFollow}>
              ...
              <button
                ...
                onClick={() => onMessageUser(viewedId)}
              >
                Messaggia
              </button>
            </>
